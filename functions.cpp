...
void deb(char *formatstring, ...)
{
	...
	// debcs->Enter();
	va_list args;
	char modname[] = "fatftpapp.exe";
	char logfile[] = "f:\\fatftpapp.txt";
	...
	if (!buff)
	{
		OutputDebugString("buf malloc() failed in deb()");
		return;
	}
	...
	char *p;
	static DWORD lastarrive = 0;
	p = strtok(buff, "\r\n");
	double difftime = (double)((double)arrivetime.HighPart - (double)lastarrive);
	lastarrive = arrivetime.HighPart;
	 ...
	 //	OutputDebugString(temp);
		...
		try
		{
			if (ss)
			{
				...
				e.arg2 = (LPVOID)ss;
				LPVOID x = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, ss + 1);
				e.type = E_APP;
				strcpy((char*)x, temp);
				e.arg = x;
				e.thread = GetCurrentThreadId();
				e.id = eventid();       e.ticks = GetTickCount();
				e.timearrive = arrivetime;
				// eventslock();
				events.push_back(e);
				eventsunlock();
			...
			// Form1->deblog->AddItem(temp, 0);
			// Form1->deblog->Scroll(0, 25);
		}
		catch(...)
		{
			OutputDebugString("except while deb()");
		}
		debnewmsgs++;
...
unsigned long heapsize(void)
{
	...
	{
		deb("heaplock: %s", fmterr());
		return 0;
	}
	// deb("locked heap %p", GetProcessHeap());
	PROCESS_HEAP_ENTRY entry;

	memset(&entry, 0, sizeof(entry));

	unsigned count = 0;
	// deb("heapsize calculating ");
	try
	{
		while (HeapWalk(GetProcessHeap(), &entry) != FALSE)
		{

			if (entry.wFlags & PROCESS_HEAP_ENTRY_BUSY)
			{
				++count;
				// if(entry.cbData>1000)
				// deb("entry %d: size: %d, overhead: %d", count, entry.cbData, entry.cbOverhead);
				size += entry.cbData;
			}
	...
	{
		fdeb("exception while walking heap");
		// fdeb("message: %s", e->Message.c_str());
	}
	HeapUnlock(GetProcessHeap());
	// deb("unlocked heap");
	// deb("heapsize=%lu", size);
	return size;
}
